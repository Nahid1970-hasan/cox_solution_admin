...
export const CostSheetTable = ({
    ...
    checkboxCellColor, // Function or object to determine checkbox cell background color
    ...rest
}) => {
    ...
    /** @checkboxCellColor => Function or object to determine checkbox cell background color based on row data (ex: (rowData) => rowData.id === 1 ? 'green' : 'yellow') */
    ...
    // Replace the checkboxBodyTemplate function with this:
        ...
        return (
            <div
                style={{
                    ...
                    height: "100%",
                    minHeight: "48px", // Match typical row height
                    margin: 0,
                    padding: "0.5rem",
                }}
            >
                <input
                    type="checkbox"
                    checked={selectedData?.some((item) => item[dataKey] === rowData[dataKey])}
                    onChange={(e) => {
                        const isChecked = e.target.checked;
                        if (isChecked) {
                            if (!selectedData?.some((item) => item[dataKey] === rowData[dataKey])) {
                                setSelectedData?.((prev) => [...(prev || []), rowData]);
                            }
                        } else {
                            setSelectedData?.((prev) => prev?.filter((item) => item[dataKey] !== rowData[dataKey]) || []);
                        }
                    }}
                    style={{
                        margin: 0,
    ...
    // And update the header style function:
                ...
                <DataTable
                    ...
                    onSelectionChange={(e, a) => {
                        if (onSelectionChange) {
                            return onSelectionChange(e, a);
                        }

                        // PrimeReact DataTable থেকে selection change handle করুন
                        if (e.type === "all") {
                            if (e.value.length === 0) {
                                return setSelectedData([]);
                            }
                        ...
                        if (a && a.originalEvent && a.originalEvent.target.type === "checkbox") {
                            setSelectedData?.(e.value);
                        }
                    }}
