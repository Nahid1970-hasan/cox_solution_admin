...
export const TableWithGroupHeader = ({
    isSelectable = true,
    allChakboox = false,
    setCustomSearch,
                ...
                >
                    ...
                    {Array.isArray(tableColumns) &&
                        tableColumns?.filter(Boolean)?.map((col, i) => {
                            if (col.headerGroup) {
                                return (
                                    <ColumnGroup key={i} header={col.header}>
                                        {col.columns.map((subCol, subIndex) => (
                                            <Column key={subIndex} {...subCol} header={subCol.header} body={subCol.sortableBody || subCol.body} footer={subCol.footer} />
                                        ))}
                                    </ColumnGroup>
                                );
                            }
                            return <Column key={i} field={col.field} header={col.header} sortable={sortable} body={col.sortableBody || col.body} footer={col.footer} {...col} />;
                        })}
